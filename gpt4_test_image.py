...
for i, example in enumerate(tqdm(winoground)):
    ...
    chat.ask(f'Is is true that "{caption_0}"? Answer with either Yes or No', conv)
    ...
    chat.ask(f'Is it true that "{caption_0}"? Answer with either Yes or No', conv)
    out_text, out_token = chat.answer(conv=conv,
                                    img_list=image_list,
                                    num_beams=beam_size,
                                    temperature=1,
    ...
    out_text = out_text.replace("\n", "\t")
    chat_responses.append(out_text)
    responses.append(f"{i},0|||No Response|||{chat_responses[0]}|||{chat_responses[1]}")
  ...
  ##################################################################################
    ...
    status = chat.upload_img(image_0, conv, image_list)
    chat.ask(f'Is it true that "{caption_1}"? Answer with either Yes or No', conv)
    out_text, out_token = chat.answer(conv=conv,
                                    img_list=image_list,
                                    num_beams=beam_size,
                                    temperature=1,
    ...
    out_text = out_text.replace("\n", "\t")
    chat_responses.append(out_text)
    responses.append(f"{i},1|||No Response|||{chat_responses[0]}|||{chat_responses[1]}")

    with open(f"outputs/responses_naive_beam{beam_size}_image_3.txt", "w") as f:
