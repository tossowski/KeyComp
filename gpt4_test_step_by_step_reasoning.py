...
responses = []
prompts = ['Describe the objects in the image', 'Explain the difference between these two captions: "{}" and "{}"', "Describe the relations between the objects you described in the image.", 'Choose the more appropriate caption: {} or {}. Answer in one sentence.']
order = [1,0,2,3]
...
for i, example in enumerate(tqdm(winoground)):
    ...
    for num in order:
        chat.ask(prompts[num].format(caption_0, caption_1), conv)
    ...
    output = "|||".join(chat_responses)
    responses.append(f"{i},0|||" + output)
    ...
    for num in order:
        ...
        chat_responses.append(out_text.replace("\n", "\t"))
    output = "|||".join(chat_responses)
    responses.append(f"{i},1|||" + output)



    with open(f"outputs/responses_{order}.txt", "w") as f:
