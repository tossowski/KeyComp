...
for i, example in enumerate(tqdm(winoground)):
    ...
    chat.ask(f'Which image is more appropriate for the caption: "{caption_0}"? Answer with A or B.', conv)
    # chat.ask(f"Select the best caption for this image:\nA:\"{caption_0}\"\nB:\"{caption_1}\"\n Start your answer with A or B. Even if you are unsure make a guess. Briefly explain your decision in 1-2 sentences.", conv)
    ...
    status = chat.upload_img(image_0, conv, image_list)
    status = chat.upload_img(image_1, conv, image_list)
    chat.ask(f'Which image is more appropriate for the caption: "{caption_1}"? Answer with A or B.', conv)
    # chat.ask(f"Select the best caption for this image:\nA:\"{caption_0}\"\nB:\"{caption_1}\"\n Start your answer with A or B. Even if you are unsure make a guess. Briefly explain your decision in 1-2 sentences.", conv)
    out_text, out_token = chat.answer(conv=conv,
                                    img_list=image_list,
                                    num_beams=beam_size,
    ...
    with open(f"outputs/image_final_test_3.txt", "w") as f:
